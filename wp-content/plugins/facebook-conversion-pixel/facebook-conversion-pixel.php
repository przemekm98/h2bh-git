...
if ( !$upgraded && $has_legacy_save ) {
	...
	define( 'FCA_PC_PLUGIN_VER', '2.3.1' );
	...
	function fca_pc_maybe_add_pixel() {
		...
		if ( $roles_check ) {
			...
			if ( function_exists('fca_pc_edd_events') ) {
				fca_pc_edd_events();
			}
			...
			if ( function_exists('fca_pc_search_integration') && !function_exists('fca_pc_woocommerce_events') && !function_exists('fca_pc_edd_events') ) {
