...
function fca_pc_plugin_menu() {
	...
	add_menu_page(
		...
		__( 'Facebook Pixel', 'facebook-conversion-pixel' ),
		'manage_options',
...
function fca_pc_settings_page() {
	...
	$html .= "<form novalidate style='display: none' action='' method='post' id='fca_pc_main_form' class='$form_class'>";
		...
		$html .= "<h1 class='nav-tab-wrapper fca-pc-nav $form_class'>";
			...
			$html .= '<a href="#" data-target="#fca_pc_integrations_table" class="nav-tab">' . __('Settings', 'facebook-conversion-pixel') . '</a>';
			$html .= '<a href="#" data-target="#fca-pc-woo-table" class="nav-tab">' . __('WooCommerce', 'facebook-conversion-pixel') . '</a>';
			$html .= '<a href="#" data-target="#fca-pc-edd-table" class="nav-tab">' . __('Easy Digital Downloads', 'facebook-conversion-pixel') . '</a>';
		...
		$html .= fca_pc_add_edd_integrations( $options );
...
function fca_pc_add_premium_integrations( $options ) {
	...
	ob_start(); ?>
	<table id='fca_pc_integrations_table' class='fca_pc_setting_table fca_pc_integrations_table'>
		<tr>
		...
		<tr>
			<th><?php _e('Track Search Event', 'facebook-conversion-pixel') ?></th>
			<td><?php echo fca_pc_input( 'search_integration', '', $search_integration_on, 'checkbox' ) ?>
			<span class='fca_pc_hint'><?php _e("Trigger Search event when a search is performed using WordPress' built-in search feature.", 'facebook-conversion-pixel') ?></span></td>
		</tr>				
		<tr>
			<th><?php _e('Track Quiz Cat Events', 'facebook-conversion-pixel') ?></th>
			<td><?php echo fca_pc_input( 'quizcat_integration', '', $quizcat_integration_on, 'checkbox' ) ?>
			<span class='fca_pc_hint'><?php _e("Send Lead event, plus custom events related to Quiz engagement to Facebook.", 'facebook-conversion-pixel') ?> <a target='_blank' href='https://fatcatapps.com/knowledge-base/pixel-cat-quizcat-integration/'><?php _e('Learn More...', 'facebook-conversion-pixel') ?></a></span></td>
		</tr>
...
function fca_pc_add_woo_integrations( $options ) {
	...
	<div id='fca-pc-woo-table'>
		<h3><?php _e('WooCommerce Integration', 'facebook-conversion-pixel') ?></h3>
		<?php if ( !$woo_active OR FCA_PC_PLUGIN_PACKAGE === 'Lite' OR FCA_PC_PLUGIN_PACKAGE === 'Personal' ) { ?>
		<h4><?php _e('Requires WooCommerce 3.0.0+ and ', 'facebook-conversion-pixel') ?><a href='https://fatcatapps.com/pixelcat/woocommerce' target='_blank'><?php _e( 'Pixel Cat Business', 'facebook-conversion-pixel' ) ?></a>.</h4>
		<?php } ?>
...
function fca_pc_add_edd_integrations( $options ) {
	...
	<div id='fca-pc-edd-table'>
		<h3><?php _e('Easy Digital Downloads Integration', 'facebook-conversion-pixel') ?></h3>
		<?php if ( !$edd_active OR FCA_PC_PLUGIN_PACKAGE === 'Lite' OR FCA_PC_PLUGIN_PACKAGE === 'Personal' ) { ?>
			<h4><?php _e('Requires Easy Digital Downloads 2.7.9+ and ', 'facebook-conversion-pixel') ?><a href='https://fatcatapps.com/pixelcat/premium' target='_blank'><?php _e( 'Pixel Cat Business', 'facebook-conversion-pixel' ) ?></a>.</h4>
		<?php } ?>
		<table class='fca_pc_integrations_table'>
			<tr>
