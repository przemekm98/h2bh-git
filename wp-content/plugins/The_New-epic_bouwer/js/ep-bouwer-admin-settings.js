(function($){
	 ...
	 * settings page.
	 *
	 * @class EPBOUWERAdminSettings
	 * @since 1.0
	 */
	EPBOUWERAdminSettings = {
		...
		_bind: function()
		{
			$('.fl-settings-nav a').on('click', EPBOUWERAdminSettings._navClicked);
			$('.fl-module-all-cb').on('click', EPBOUWERAdminSettings._moduleAllCheckboxClicked);
			$('.fl-module-cb').on('click', EPBOUWERAdminSettings._moduleCheckboxClicked);
			$('.fl-override-ms-cb').on('click', EPBOUWERAdminSettings._overrideCheckboxClicked);
			$('input[name=fl-upload-icon]').on('click', EPBOUWERAdminSettings._showIconUploader);
			$('.fl-delete-icon-set').on('click', EPBOUWERAdminSettings._deleteCustomIconSet);
			$('#uninstall-form').on('submit', EPBOUWERAdminSettings._uninstallFormSubmit);
			$('input[name=fl-help-button-enabled]').on('click', EPBOUWERAdminSettings._initHelpButtonSettings);
			$('input[name=fl-help-video-enabled]').on('click', EPBOUWERAdminSettings._initHelpButtonSettings);
			$('input[name=fl-knowledge-base-enabled]').on('click', EPBOUWERAdminSettings._initHelpButtonSettings);
			$('input[name=fl-forums-enabled]').on('click', EPBOUWERAdminSettings._initHelpButtonSettings);
		},
		...
		{
			$('.fl-override-ms-cb').each(EPBOUWERAdminSettings._initOverride);
		},
		...
		{
			if(EPBOUWERAdminSettings._iconUploader === null) {
				EPBOUWERAdminSettings._iconUploader = wp.media({
					title: EPBOUWERAdminSettingsStrings.selectFile,
					button: { text: EPBOUWERAdminSettingsStrings.selectFile },
					library : { type : 'application/zip' },
					multiple: false
				});
			}
			
			EPBOUWERAdminSettings._iconUploader.once('select', $.proxy(EPBOUWERAdminSettings._iconFileSelected, this));
			EPBOUWERAdminSettings._iconUploader.open();
		},
		...
		{
			var file = EPBOUWERAdminSettings._iconUploader.state().get('selection').first().toJSON();
		...
		{
			var result = prompt(EPBOUWERAdminSettingsStrings.uninstall, '');
