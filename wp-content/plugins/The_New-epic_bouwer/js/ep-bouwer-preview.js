(function($){
	 ...
	 *
	 * @class EPBOUWERPreview
	 * @since 1.3.3
	 */
	EPBOUWERPreview = function(o)
	{
		// Type
		this.type = o.type;
		...
		if(o.layout != 'undefined' && o.layout) {
			EPBOUWER._renderLayout(o.layout, $.proxy(this._init, this));
		}
		else {
			this._init();
		}
	...
	EPBOUWERPreview.prototype = {
		...
		{
			// Node Id
			this.nodeId = $('.ep-bouwer-settings').data('node');
		...
		_initElementsAndClasses: function()
		{
			...
			// Class Names
			$.extend(this.classes, {
				settings        : '.ep-bouwer-' + this.type + '-settings',
				settingsHeader  : '.ep-bouwer-' + this.type + '-settings .fl-lightbox-header',
				node            : EPBOUWER._contentClass + ' .fl-node-' + this.nodeId,
				content         : EPBOUWER._contentClass + ' .fl-node-' + this.nodeId + ' ' + contentClass
			});
		...
		_saveState: function() 
		{
			var post    = $('#fl-post-id').val(),
				css     = $('#ep-bouwer-layout-' + post + '-css').attr('href'),
				js      = $('script[src*="/cache/' + post + '"]').attr('src'),
				html    = $(EPBOUWER._contentClass).html();
		...
		preview: function() 
		{
			...
			this._xhr = EPBOUWER.ajax({
				action          : 'ep_bouwer_render_preview',
				node_id         : nodeId,
		...
		delayPreview: function(e)
		{
			var heading         = typeof e == 'undefined' ? [] : $(e.target).closest('tr').find('th'),
				widgetHeading   = $('.ep-bouwer-widget-settings .ep-bouwer-settings-title'),
				lightboxHeading = $('.ep-bouwer-settings .fl-lightbox-header'),
				loaderSrc       = flBuilderUrl + 'img/ajax-loader-small.gif',
				loader          = $('<img class="ep-bouwer-preview-loader" src="' + loaderSrc + '" />');
			
			$('.ep-bouwer-preview-loader').remove();
		...
		_renderPreview: function(response) 
		{
			...
			EPBOUWER._renderLayout(response, $.proxy(this._renderPreviewComplete, this));
		},
		...
		_renderPreviewComplete: function() 
		{
			...
			$( EPBOUWER._contentClass ).trigger( 'ep-bouwer.preview-rendered' );
		},
		...
		revert: function() 
		{
			...
			// Render the layout.
			EPBOUWER._renderLayout(this.state);
		},
