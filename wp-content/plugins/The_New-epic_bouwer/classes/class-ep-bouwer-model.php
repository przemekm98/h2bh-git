...
final class EPBOUWERModel {
	 ...
	 * or from the ep_bouwer_data post variable.
	 *
	 * @since 1.0 
	 * @access private
	 * @var array $post_data
	...
	static public function update($method = null, $params = array())
	{
		...
		// Method exists.
		else if(method_exists('EPBOUWERModel', $method)) {

			// Call the method.
			$result = call_user_func_array('EPBOUWERModel::' . $method, $params);
	...
	static public function get_edit_url( $post_id = false )
	{
		...
		$url = add_query_arg( 'ep_bouwer', '', get_permalink( $post->ID ) );
	 ...
	 * link using the ep_bouwer_upgrade_url filter.
	 *
	 * @since 1.0 
	...
	{
		$url = EP_BOUWER_UPGRADE_URL . '?' . http_build_query( $params, '', '&' );
		
		return apply_filters( 'ep_bouwer_upgrade_url', $url );
	}

	/**
	 * Returns an array of post data from either $_POST['ep_bouwer_data']
	...
	static public function get_post_data()
	{
		if(!self::$post_data) {
			...
			if(isset($_POST['ep_bouwer_data'])) {
				
				$data = EPBOUWERUtils::json_decode_deep( wp_unslash( $_POST['ep_bouwer_data'] ) );
	...
	{
		$value = self::get_admin_settings_option( '_ep_bouwer_post_types', true );

		if ( ! $value ) {
			return array( 'page', 'ep-bouwer-template' );
		}
		else {
			$value[] = 'ep-bouwer-template';
			return $value;
		}
	}
	...
	{
		return apply_filters('ep_bouwer_global_posts', array());
	}
	...
	{
		if(isset($data['ep_bouwer_post_lock'])) {

			require_once ABSPATH . 'wp-admin/includes/post.php';

			wp_set_post_lock($data['ep_bouwer_post_lock']['post_id']);
		}
	}
	...
	static public function is_builder_enabled()
	{
		if(!is_admin() && post_password_required()) {
			...
			if($post && in_array($post->post_type, $post_types)) {
				return get_post_meta(self::get_post_id(), '_ep_bouwer_enabled', true);
			}
		}
	...
	static public function is_builder_active()
	{
		if ( self::is_post_editable() && ! is_admin() && ! post_password_required() ) {
			...
			else if ( isset( $post_data['ep_bouwer'] ) ) {
				return true;
			}
		}
	...
	static public function is_new_user()
	{
		if ( self::is_builder_active() ) {
			...
			$launched		= get_user_meta( $current_user->ID, '_ep_bouwer_launched', true );
			
			if ( empty( $launched ) ) {
				update_user_meta( $current_user->ID, '_ep_bouwer_launched', 1 );
				return true;
			}
		}
	...
	{
		update_post_meta(self::get_post_id(), '_ep_bouwer_enabled', true);
	}
	...
	{
		update_post_meta(self::get_post_id(), '_ep_bouwer_enabled', false);
	}
	...
	static public function get_upload_dir()
	{
		...
		$dir_name = basename( EP_BOUWER_DIR );
		
		// We use ep-plugin for the lite version as well.
		if ( $dir_name == 'epic-bouwer-lite-version' ) {
			$dir_name = 'ep-plugin';
		}
	...
	static public function get_nodes($type = null, $parent_id = null, $status = null)
	{
		...
		// Return nodes of a certain type.
			...
			uasort($nodes, array('EPBOUWERModel', 'order_nodes'));
		}
	...
	static public function process_row_settings($row, $new_settings)
	{
		...
		if($new_settings->bg_type == 'video') {
			...
			// Video
			$video = EPBOUWERPhoto::get_attachment_data($new_settings->bg_video);
	...
	static public function load_modules()
	{
		...
		while(false !== ($entry = $dir->read())) {
			...
			$child_path		= get_stylesheet_directory() . '/ep-bouwer/modules/' . $module_path;
			$theme_path		= get_template_directory() . '/ep-bouwer/modules/' . $module_path;
			$builder_path	= EP_BOUWER_DIR . 'modules/' . $module_path;
	...
	static public function register_module($class, $form)
	{
		if(class_exists($class)) {
			...
			if ( isset( self::$modules[ $instance->slug ] ) ) {
				error_log( sprintf( _x( 'A module with the filename %s.php already exists! Please namespace your module filenames to ensure compatibility with Epic Bouwer.', '%s stands for the module filename', 'ep-bouwer' ), $instance->slug ) );
				return;
			}
			
			// Filter the enabled flag.
			$instance->enabled = apply_filters( 'ep_bouwer_register_module', $instance->enabled, $instance );
	...
	static public function get_enabled_modules()
	{
		...
		$default[]	= 'all';
		$setting 	= self::get_admin_settings_option( '_ep_bouwer_enabled_modules', true );
	...
	static public function get_categorized_modules( $show_disabled = false )
	{
		...
		$basic_key		 = __('Basic Modules', 'ep-bouwer');
		$advanced_key	 = __('Advanced Modules', 'ep-bouwer');
		$other_key		 = __('Other Modules', 'ep-bouwer');
		$widgets_key	 = __('WordPress Widgets', 'ep-bouwer');
	...
	{
		require_once EP_BOUWER_DIR . 'includes/global-settings.php';
		require_once EP_BOUWER_DIR . 'includes/row-settings.php';
		require_once EP_BOUWER_DIR . 'includes/column-settings.php';
		require_once EP_BOUWER_DIR . 'includes/module-settings.php';
		
		$user_templates = EP_BOUWER_DIR . 'includes/user-template-settings.php';
	...
	static public function get_settings_form_defaults( $type )
	{
		...
		// Cache the defaults.
		self::$settings_form_defaults[ $type ] = apply_filters( 'ep_bouwer_settings_form_defaults', $defaults, $form_type );
	...
	static public function save_global_settings($settings = array())
	{
		...
		return update_option('_ep_bouwer_settings', $settings);
	}
	...
	static public function duplicate_post()
	{
		...
		$data = array(
			...
			'post_status'	 => 'draft',
			'post_title'	 => sprintf( _x( 'Copy of %s', '%s stands for post/page title.', 'ep-bouwer' ), $post->post_title ),
	...
	static public function get_layout_data($status = null, $post_id = null)
	{
		...
		if($status == 'published') {
			if(isset(self::$published_layout_data[$post_id])) {
				...
				$data = get_metadata('post', $post_id, '_ep_bouwer_data', true);
		...
		// Get draft data?
			...
			if(isset(self::$draft_layout_data[$post_id])) {
				...
				$data = get_metadata('post', $post_id, '_ep_bouwer_draft', true);
	...
	static public function update_layout_data($data, $status = null, $post_id = null)
	{
		...
		if($status == 'published') {
			update_metadata('post', $post_id, '_ep_bouwer_data', $data);
			self::$published_layout_data[$post_id] = $data;
		}
		// Update draft data?
		else if($status == 'draft') {
			update_metadata('post', $post_id, '_ep_bouwer_draft', $data);
	...
	{
		$editor_content = EPBOUWER::render_editor_content();
		$post_id		= self::get_post_id();
	...
	static public function duplicate_wpml_layout($original_post_id = null, $new_post_id = null)
	{
		...
		$enabled			= get_post_meta($original_post_id, '_ep_bouwer_enabled', true);
		...
		if(!empty($enabled)) {
			update_post_meta($new_post_id, '_ep_bouwer_enabled', true);
	...
	{
		$value = self::get_admin_settings_option( '_ep_bouwer_enabled_templates', true );
	...
	{
		$value = self::get_admin_settings_option( '_ep_bouwer_user_templates_admin', true );
	...
	static public function save_user_template()
	{
		...
		update_post_meta($post_id, '_ep_bouwer_enabled', true);
	}
	...
	static public function get_user_templates()
	{
		$categorized = array(
			'uncategorized' => array(
				'name'		=> _x( 'Uncategorized', 'Default user template category.', 'ep-bouwer' ),
				'templates'	=> array()
			)
		);
		...
		foreach( $posts as $post ) {
			...
			if ( has_post_thumbnail( $post->ID ) ) {
				...
				$image = EP_BOUWER_URL . 'img/templates/blank.jpg';
			}
		...
		foreach ( $templates as $template ) {
			...
			$cats = wp_get_post_terms( $template['id'], 'ep-bouwer-template-category' );
	...
	static public function apply_template($index = 0, $append = false)
	{
		...
		if ( class_exists( 'EPBOUWERTemplatesOverride' ) ) {
			...
			$success = EPBOUWERTemplatesOverride::apply( $index, $append );
	...
	{
		$templates = unserialize(file_get_contents(EP_BOUWER_DIR . 'data/templates.dat'));
	...
	static public function save_templates($templates = array())
	{
		...
		// Save the templates array.
		file_put_contents(EP_BOUWER_DIR . 'data/templates.dat', serialize($updated));
	}
	...
	static public function get_template_selector_data()
	{
		...
		if ( class_exists( 'EPBOUWERTemplatesOverride' ) ) {
			...
			$data = EPBOUWERTemplatesOverride::get_selector_data();
		...
		$category_labels = array(
			'landing' => __( 'Home Pages', 'ep-bouwer' ),
			'company' => __( 'Content Pages', 'ep-bouwer' )
		);
		$categorized = array();
		$templates   = array();
		...
		foreach( self::get_templates() as $key => $template ) {
			$templates[] = array(
				'id' 		=> $key,
				'name'  	=> $template->name,
				'image' 	=> EP_BOUWER_URL . 'img/templates/' . $template->image,
	...
	{
		$value = self::get_admin_settings_option( '_ep_bouwer_branding', false );
		
		return ! $value ? __( 'Page Builder', 'ep-bouwer' ) : stripcslashes( $value );
	}
	...
	{
		$value = self::get_admin_settings_option( '_ep_bouwer_branding_icon', false );
		
		return false === $value ? EP_BOUWER_URL . 'img/epic.png' : $value;
	}
	...
	{
		$value = self::get_admin_settings_option( '_ep_bouwer_enabled_icons', true );
	...
	{
		$value = self::get_admin_settings_option( '_ep_bouwer_editing_capability', true );
	...
	static public function get_help_button_defaults()
	{
		$defaults = array(
			...
			'tour'					=> true,
			'video'					=> true,
			'video_embed'			=> '<iframe src="https://www.youtube.com/embed/zz7ydcC6y24" width="420" height="315" frameborder="0" webkitallowfullscreen mozallowfullscreen allowfullscreen></iframe>',
			'knowledge_base'		=> true,
			'knowledge_base_url'	=> 'http://www.epicwebsite.nl/epicbouwer/knowledge-base/?utm_source=external&utm_medium=builder&utm_campaign=docs-button',
			'forums'				=> true,
			'forums_url'			=> 'http://www.epicwebsite.nl/epicbouwer/support/?utm_source=external&utm_medium=builder&utm_campaign=forums-button',
			'meer_info'				=> true,
			'meer_info '			=> 'http://www.epicwebsite.nl/epicbouwer/info',
	...
	{
		$value = self::get_admin_settings_option( '_ep_bouwer_help_button', false );
	...
	{
		return get_option( '_ep_bouwer_services', array() );
	}
	...
	static public function update_services( $service, $account, $data )
	{
		...
		update_option( '_ep_bouwer_services', $services );
	}
	...
	static public function get_admin_settings_option( $key, $network_override = true )
	{
		...
		else if ( ! $network_override && class_exists( 'EPBOUWERMultisiteSettings' ) ) {
			$value = get_site_option( $key );
		}
		// Network overrides are allowed. Return the subsite option if it exists.
		else if ( class_exists( 'EPBOUWERMultisiteSettings' ) ) {
	...
	static public function update_admin_settings_option( $key, $value, $network_override = true )
	{
		...
		else if ( $network_override && EPBOUWERAdminSettings::multisite_support() && ! isset( $_POST['fl-override-ms'] ) ) {
			delete_option( $key );
		}
	...
	/**
	 * Returns the plugin basename for Epic Bouwer.
	 *
	 * @since 1.0
	 * @return string
	 */
	...
	{
		return plugin_basename( EP_BOUWER_DIR . 'ep-bouwer.php' );
	}

	/**
	 * Deletes almost all database data and asset cache for the builder.
	 * We don't delete _ep_bouwer_enabled, _ep_bouwer_data and _ep_bouwer_draft
	...
	static public function uninstall_database()
	{
		if(current_user_can('delete_plugins')) {
			...
			delete_option('_ep_bouwer_settings');
			delete_option('_ep_bouwer_enabled_modules');
			delete_option('_ep_bouwer_enabled_templates');
			delete_option('_ep_bouwer_user_templates_admin');
			delete_option('_ep_bouwer_post_types');
			delete_option('_ep_bouwer_enabled_icons');
			delete_option('_ep_bouwer_branding');
			delete_option('_ep_bouwer_branding_icon');
			delete_option('_ep_bouwer_editing_capability');
			delete_option('_ep_bouwer_help_button');
			
			// Delete builder user meta.
			delete_metadata('user', 0, '_ep_bouwer_launched', 1, true);
