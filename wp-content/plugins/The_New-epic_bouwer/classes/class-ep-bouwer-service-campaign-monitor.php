 ...
 * @since 1.5.4
 */
final class EPBOUWERServiceCampaignMonitor extends EPBOUWERService {
	...
	public function __construct() 
	{
		if ( ! class_exists( 'CS_REST_General' ) ) {
			require_once EP_BOUWER_DIR . 'includes/vendor/campaign-monitor/csrest_general.php';
			require_once EP_BOUWER_DIR . 'includes/vendor/campaign-monitor/csrest_clients.php';
			require_once EP_BOUWER_DIR . 'includes/vendor/campaign-monitor/csrest_lists.php';
			require_once EP_BOUWER_DIR . 'includes/vendor/campaign-monitor/csrest_subscribers.php';
		}
	}
	...
	public function connect( $fields = array() ) 
	{
		...
		// Try to connect and store the connection data.
			...
			if ( $result->was_successful() ) {
				...
				$response['error'] = __( 'Error: Please check your API key.', 'ep-bouwer' );
			}
		}
	...
	public function render_connect_settings() 
	{
		...
		EPBOUWER::render_settings_field( 'api_key', array(
			...
			'help'          => __( 'Your API key can be found in your Campaign Monitor account under Account Settings > API Key.', 'ep-bouwer' ),
			'preview'       => array(
				'type'          => 'none'
			)
		)); 
	...
	private function render_client_field( $clients, $settings ) 
	{
		...
		EPBOUWER::render_settings_field( 'client_id', array(
			'row_class'     => 'ep-bouwer-service-field-row',
			'class'         => 'ep-bouwer-campaign-monitor-client-select',
			'type'          => 'select',
			'label'         => _x( 'Client', 'A client account in Campaign Monitor.', 'ep-bouwer' ),
			'options'       => $options,
			'preview'       => array(
				'type'          => 'none'
			)
	...
	public function subscribe( $settings, $email, $name = false )
	{
		...
		if ( ! $account_data ) {
			$response['error'] = __( 'There was an error subscribing to Campaign Monitor. The account is no longer connected.', 'ep-bouwer' );
		}
		else {
			...
			if ( ! $result->was_successful() ) {
				$response['error'] = __( 'There was an error subscribing to Campaign Monitor.', 'ep-bouwer' );
			}
		}
