 ...
 * @since 1.5.4
 */
final class EPBOUWERServiceAWeber extends EPBOUWERService {
	...
	public function get_api( $auth_code ) 
	{
		...
		if ( ! class_exists( 'AWeberAPI' ) ) {
			require_once EP_BOUWER_DIR . 'includes/vendor/aweber/aweber_api.php';
		}
	...
	public function connect( $fields = array() ) 
	{
		...
		if ( ! isset( $fields['auth_code'] ) || empty( $fields['auth_code'] ) ) {
			$response['error'] = __( 'Error: You must provide an Authorization Code.', 'ep-bouwer' );
		}
		// Make sure we have a valid authorization code.
		else if ( 6 != count( explode( '|', $fields['auth_code'] ) ) ) {
			$response['error'] = __( 'Error: Please enter a valid Authorization Code.', 'ep-bouwer' );
		}
	...
	public function render_connect_settings() 
	{
		...
		EPBOUWER::render_settings_field( 'auth_code', array(
			'row_class'     => 'ep-bouwer-service-connect-row',
			'class'         => 'ep-bouwer-service-connect-input',
			'type'          => 'text',
			'label'         => __( 'Authorization Code', 'ep-bouwer' ),
			'description'   => sprintf( __( 'Please register this website with AWeber to get your Authorization Code. <a%s>Register Now</a>', 'ep-bouwer' ), ' href="https://auth.aweber.com/1.0/oauth/authorize_app/baa1f131" target="_blank"' ),
			'preview'       => array(
				'type'          => 'none'
			)
		)); 
	...
	public function subscribe( $settings, $email, $name = false )
	{
		...
		if ( ! $account_data ) {
			$response['error'] = __( 'There was an error subscribing to AWeber. The account is no longer connected.', 'ep-bouwer' );
		}
		else {
			...
			try {
				...
				if ( is_array( $result ) && isset( $result['Status-Code'] ) && 201 == $result['Status-Code'] ) {
					...
					$response['error'] = __( 'There was an error connecting to AWeber. Please try again.', 'ep-bouwer' );
				}
			} 
			catch ( AWeberAPIException $e ) {
				$response['error'] = sprintf(
					__( 'There was an error subscribing to AWeber. %s', 'ep-bouwer' ),
					$e->getMessage()
				);
			}
		}
