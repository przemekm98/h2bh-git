 ...
 * @since 1.5.2
 */
final class EPBOUWERServiceMadMimi extends EPBOUWERService {
	...
	public function get_api( $api_email, $api_key ) 
	{
		...
		if ( ! class_exists( 'MadMimi' ) ) {
			require_once EP_BOUWER_DIR . 'includes/vendor/madmimi/MadMimi.class.php';
		}
	...
	public function connect( $fields = array() ) 
	{
		...
		// Try to connect and store the connection data.
			...
			if ( ! simplexml_load_string( $api->Lists() ) ) {
				$response['error'] = __( 'Unable to connect to Mad Mimi. Please check your credentials.', 'ep-bouwer' );
			}
			else {
				$response['data'] = array( 
	...
	public function render_connect_settings() 
	{
		...
		EPBOUWER::render_settings_field( 'api_email', array(
			'row_class'     => 'ep-bouwer-service-connect-row',
			'class'         => 'ep-bouwer-service-connect-input',
			'type'          => 'text',
			'label'         => __( 'Email Address', 'ep-bouwer' ),
			'help'          => __( 'The email address associated with your Mad Mimi account.', 'ep-bouwer' ),
			'preview'       => array(
				'type'          => 'none'
			)
		)); 
		...
		EPBOUWER::render_settings_field( 'api_key', array(
			...
			'help'          => __( 'Your API key can be found in your Mad Mimi account under Account > Settings &amp; Billing > API.', 'ep-bouwer' ),
			'preview'       => array(
				'type'          => 'none'
			)
		)); 
	...
	public function render_fields( $account, $settings ) 
	{
		...
		if ( ! $result ) {
			$response['error'] = __( 'There was a problem retrieving your lists. Please check your API credentials.', 'ep-bouwer' );
		}
		else {
	...
	public function subscribe( $settings, $email, $name = false )
	{
		...
		if ( ! $account_data ) {
			$response['error'] = __( 'There was an error subscribing to Mad Mimi. The account is no longer connected.', 'ep-bouwer' );
		}
		else {
