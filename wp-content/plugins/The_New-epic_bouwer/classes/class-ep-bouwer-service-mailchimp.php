 ...
 * @since 1.5.4
 */
final class EPBOUWERServiceMailChimp extends EPBOUWERService {
	...
	public function get_api( $api_key ) 
	{
		...
		if ( ! class_exists( 'Mailchimp' ) ) {
			require_once EP_BOUWER_DIR . 'includes/vendor/mailchimp/mailchimp.php';
		}
	...
	public function render_connect_settings() 
	{
		...
		EPBOUWER::render_settings_field( 'api_key', array(
			...
			'help'          => __( 'Your API key can be found in your MailChimp account under Account > Extras > API Keys.', 'ep-bouwer' ),
			'preview'       => array(
				'type'          => 'none'
			)
		)); 
	...
	private function render_list_field( $lists, $settings ) 
	{
		...
		EPBOUWER::render_settings_field( 'list_id', array(
			...
			'class'         => 'ep-bouwer-service-list-select ep-bouwer-mailchimp-list-select',
			'type'          => 'select',
	...
	private function render_groups_field( $list_id, $groups, $settings ) 
	{
		...
		$options = array( '' => __( 'No Group', 'ep-bouwer' ) );
		...
		EPBOUWER::render_settings_field( 'groups', array(
			'row_class'     => 'ep-bouwer-service-field-row',
			'class'         => 'ep-bouwer-mailchimp-group-select',
			'type'          => 'select',
			'label'         => _x( 'Groups', 'MailChimp list group.', 'ep-bouwer' ),
			'multi-select'	=> true,
			'options'       => $options,
			'preview'       => array(
				'type'          => 'none'
	...
	public function subscribe( $settings, $email, $name = false )
	{
		...
		if ( ! $account_data ) {
			$response['error'] = __( 'There was an error subscribing to MailChimp. The account is no longer connected.', 'ep-bouwer' );
		}
		else {
			
			$api     = $this->get_api( $account_data['api_key'] );
			$double  = apply_filters( 'ep_bouwer_mailchimp_double_option', true );
			$welcome = apply_filters( 'ep_bouwer_mailchimp_welcome', true );
			...
			try {
				...
				$response['error'] = sprintf(
					__( 'There was an error subscribing to MailChimp. %s', 'ep-bouwer' ),
					$e->getMessage()
				);
			}
		}
