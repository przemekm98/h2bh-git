 ...
 * @since 1.5.8
 */
final class EPBOUWERServiceHatchbuck extends EPBOUWERService {
	...
	public function render_connect_settings() 
	{
		...
		EPBOUWER::render_settings_field( 'api_key', array(
			...
			'help'          => __( 'Your API key can be found in your Hatchbuck account under Account Settings > Web API.', 'ep-bouwer' ),
			'preview'       => array(
				'type'          => 'none'
			)
		)); 
	...
	private function render_tag_field( $settings ) 
	{
		...
		EPBOUWER::render_settings_field( 'list_id', array(
			...
			'type'          => 'text',
			'label'         => _x( 'Tag', 'A tag to add to contacts in Hatchbuck when they subscribe.', 'ep-bouwer' ),
			'preview'       => array(
				'type'          => 'none'
			)
		), $settings); 
	...
	public function subscribe( $settings, $email, $name = false )
	{
		...
		if ( ! $account_data ) {
			$response['error'] = __( 'There was an error subscribing to Hatchbuck. The account is no longer connected.', 'ep-bouwer' );
		}
		else {
			...
			if ( 401 == $result['response']['code'] ) {
				$response['error'] = __( 'There was an error subscribing to Hatchbuck. The API key is invalid.', 'ep-bouwer' );
			...
			// Generic error. Contact not found should be 400.
				...
				$response['error'] = __( 'There was an error subscribing to Hatchbuck.', 'ep-bouwer' );
				return $response;
			}
