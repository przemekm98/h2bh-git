...
final class EPBOUWERIcons {
	...
	static public function get_sets()
	{
		...
		if ( is_multisite()) {
			...
			$enabled_icons	= get_option( '_ep_bouwer_enabled_icons' );
	...
	static private function register_core_sets()
	{
		...
		foreach ( self::$sets as $set_key => $set_data ) {
			if ( 'core' == $set_data['type'] ) {
				$icons = json_decode( file_get_contents( EP_BOUWER_DIR . 'json/' . $set_key . '.json' ) );
	...
	static private function register_custom_sets()
	{
		// Get uploaded sets.
		$enabled_icons = EPBOUWERModel::get_enabled_icons();
		$upload_info   = EPBOUWERModel::get_cache_dir( 'icons' );
	...
	{
		$fields = EPBOUWERModel::get_settings_form_fields( $module->form );
		
		foreach ( $fields as $name => $field ) {
			if ( isset( $field['multiple'] ) && true === $field['multiple'] ) {
				$form = EPBOUWERModel::$settings_forms[ $field['form'] ];
	...
	{
		$fields = EPBOUWERModel::get_settings_form_fields( $form );
	...
	static private function enqueue_custom_styles_by_key( $key )
	{
		...
		if ( isset( $sets[ $key ] ) ) {
			...
			if ( 'icomoon' == $set['type'] ) {
				wp_enqueue_style( $key, $set['stylesheet'], array(), EP_BOUWER_VERSION );
			}
