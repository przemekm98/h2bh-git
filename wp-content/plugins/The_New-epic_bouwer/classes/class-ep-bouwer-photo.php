...
final class EPBOUWERPhoto {
	...
	{
		if ( empty( $photo ) ) {
			echo EP_BOUWER_URL . 'img/spacer.png';
		}
	...
	static public function get_src_options($selected, $photo)
	{
		if ( ! isset( $photo->sizes ) ) {
			echo '<option value="' . $photo->url . '" selected="selected">' . _x( 'Full Size', 'Image size.', 'ep-bouwer' ) . '</option>';   
		}
		else {
			
			$titles = array(
				'full'      => _x( 'Full Size', 'Image size.', 'ep-bouwer' ),
				'large'     => _x( 'Large', 'Image size.', 'ep-bouwer' ),
				'medium'    => _x( 'Medium', 'Image size.', 'ep-bouwer' ),
				'thumbnail' => _x( 'Thumbnail', 'Image size.', 'ep-bouwer' )
			);
