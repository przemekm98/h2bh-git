...
final class EPBOUWER {
	...
	static public function load_plugin_textdomain()
	{
		...
		$locale = apply_filters( 'plugin_locale', get_locale(), 'ep-bouwer' );

		//Setup paths to current locale file
		$mofile_local  = trailingslashit( EP_BOUWER_DIR ) . 'languages/' . $locale . '.mo';
		$mofile_global = trailingslashit( WP_LANG_DIR ) . 'plugins/epic-bouwer/' . $locale . '.mo';

		if ( file_exists( $mofile_local ) ) {
			//Look in local /wp-content/plugins/epic-bouwer/languages/ folder
			return load_textdomain( 'ep-bouwer', $mofile_local );
		} 
		else if ( file_exists( $mofile_global ) ) {
			//Look in global /wp-content/languages/plugins/epic-bouwer/ folder
			return load_textdomain( 'ep-bouwer', $mofile_global );
		}
	...
	static public function init()
	{
		...
		if(EPBOUWERModel::is_builder_active()) {
			...
			$post_id = EPBOUWERModel::get_post_id();
	...
	{
		EPBOUWERModel::register_module($class, $form);
	}
	...
	{
		EPBOUWERModel::register_settings_form($id, $form);
	}
	...
	{
		return EPBOUWERModel::is_builder_active() ? 'tinymce' : $type;
	}
	...
	static public function add_editor_css($mce_css)
	{
		if(EPBOUWERModel::is_builder_active()) {
			...
			$mce_css .= EP_BOUWER_URL . 'css/editor.css';
		}
	...
	static public function include_jquery()
	{
		if(EPBOUWERModel::is_builder_enabled()) {
			include EP_BOUWER_DIR . 'includes/jquery.php';
		}
	}
	...
	static public function layout_styles_scripts()
	{
		...
		$original_post  = $post;
		$ver            = EP_BOUWER_VERSION;
		$css_url        = EP_BOUWER_URL . 'css/';
		$js_url         = EP_BOUWER_URL . 'js/';
		...
		// Enqueue assets for posts via the ep_bouwer_global_posts filter.
		$post_ids = EPBOUWERModel::get_global_posts();
	...
	static public function enqueue_layout_styles_scripts($post_id)
	{
		if(EPBOUWERModel::is_builder_enabled()) {
			...
			$rows       = EPBOUWERModel::get_nodes('row');
			$modules    = EPBOUWERModel::get_all_modules();
			$asset_info = EPBOUWERModel::get_asset_info();
			$asset_ver  = EPBOUWERModel::get_asset_version();
			...
			$deps = apply_filters( 'ep_bouwer_layout_style_dependencies', array() );

			wp_enqueue_style('ep-bouwer-layout-' . $post_id, $asset_info['css_url'], $deps, $asset_ver);
			...
			wp_enqueue_script('ep-bouwer-layout-' . $post_id, $asset_info['js_url'], array('jquery'), $asset_ver, true);
		}
	}
	...
	static public function styles_scripts()
	{
		if(EPBOUWERModel::is_builder_active()) {
			...
			wp_enqueue_style('ep-bouwer',              $css_url . 'ep-bouwer.css', array(), $ver);
			
			/* Custom Icons */
			EPBOUWERIcons::enqueue_all_custom_icons_styles();

			/* RTL Support */
			if(is_rtl()) {
				wp_enqueue_style('ep-bouwer-rtl',      $css_url . 'ep-bouwer-rtl.css', array(), $ver);
			}
			...
			wp_enqueue_script('ep-bouwer',             $js_url . 'ep-bouwer.js', array(), $ver, true);
			wp_enqueue_script('ep-bouwer-preview',     $js_url . 'ep-bouwer-preview.js', array(), $ver, true);
			wp_enqueue_script('ep-bouwer-services',    $js_url . 'ep-bouwer-services.js', array(), $ver, true);
			wp_enqueue_script('ep-bouwer-tour',        $js_url . 'ep-bouwer-tour.js', array(), $ver, true);

			/* Core template settings */
			if(file_exists(EP_BOUWER_DIR . 'js/ep-bouwer-template-settings.js')) {
				wp_enqueue_script('ep-bouwer-template-settings', EP_BOUWER_URL . 'js/ep-bouwer-template-settings.js', array(), $ver, true);
			}

			/* Additional module styles and scripts */
			foreach(EPBOUWERModel::$modules as $module) {
	...
	static public function body_class($classes)
	{
		...
		if(EPBOUWERModel::is_builder_active() && !current_user_can(EPBOUWERModel::get_editing_capability())) {
			$classes[] = 'ep-bouwer-simple';
		}
	...
	static public function admin_bar_menu($wp_admin_bar)
	{
		...
		if ( EPBOUWERModel::is_post_editable() ) {

			$wp_admin_bar->add_node( array(
				'id'    => 'ep-bouwer-frontend-edit-link',
				'title' => '<style> #wp-admin-bar-ep-bouwer-frontend-edit-link .ab-icon:before { content: "\f116" !important; top: 2px; margin-right: 3px; } </style><span class="ab-icon"></span>' . EPBOUWERModel::get_branding(),
				'href'  => EPBOUWERModel::get_edit_url( $wp_the_query->post->ID )
			));
		}
	}
	...
	static public function render_ui()
	{
		...
		if ( EPBOUWERModel::is_builder_active() ) {
			...
			$categories         = EPBOUWERModel::get_categorized_modules();
			$enabled_templates  = EPBOUWERModel::get_enabled_templates();
			$help_button        = EPBOUWERModel::get_help_button_settings();

			include EP_BOUWER_DIR . 'includes/ui.php';
			include EP_BOUWER_DIR . 'includes/js-config.php';
		}
	}
	 ...
	 * defined in ep-bouwer.php. To output builder content, use 
	...
	static public function render_content( $content )
	{
		...
		$in_loop        = in_the_loop();
		$is_global      = in_array( $post_id, EPBOUWERModel::get_global_posts() );
		...
		if( $enabled && ! $rendering && ! $ajax && ( $in_loop || $is_global ) ) {
			...
			remove_filter( 'the_content', 'EPBOUWER::render_content' );

			// Render the content.
			ob_start();
			echo '<div class="ep-bouwer-content ep-bouwer-content-' . $post_id . '" data-post-id="' . $post_id . '">';
			self::render_rows();
			echo '</div>';
	...
	static public function render_editor_content()
	{
		...
		foreach($rows as $row) {
			...
			$groups = EPBOUWERModel::get_nodes('column-group', $row->node);

			foreach($groups as $group) {

				$cols = EPBOUWERModel::get_nodes('column', $group->node);

				foreach($cols as $col) {

					$modules = EPBOUWERModel::get_modules($col->node);
					...
					foreach($modules as $module) {
						...
						if($module->editor_export) {
							...
							EPBOUWER::render_module_html($module->settings->type, $module->settings, $module);
						}
					}
				}
			}
	...
	static public function render_settings($form = array(), $settings, $return = false)
	{
		...
		ob_start();
		include EP_BOUWER_DIR . 'includes/settings.php';
		$html = ob_get_clean();
	...
	static public function render_settings_field($name, $field, $settings = null)
	{
		...
		if($is_multiple && $supports_multiple) {
			...
			echo '<tbody class="fl-field ep-bouwer-field-multiples" data-type="form" data-preview=\'' . $preview . '\'>';

			for($i = 0; $i < count($values); $i++) {
				$value = $values[$i];
				echo '<tr class="ep-bouwer-field-multiple" data-field="'. $arr_name .'">';
				include EP_BOUWER_DIR . 'includes/field.php';
				echo '<td class="ep-bouwer-field-actions">';
				echo '<i class="ep-bouwer-field-move fa fa-arrows"></i>';
				echo '<i class="ep-bouwer-field-copy fa fa-copy"></i>';
				echo '<i class="ep-bouwer-field-delete fa fa-times"></i>';
				echo '</td>';
				echo '</tr>';
			}
			...
			echo '<a href="javascript:void(0);" onclick="return false;" class="ep-bouwer-field-add ep-bouwer-button" data-field="'. $arr_name .'">'. sprintf( _x( 'Add %s', 'Field name to add.', 'ep-bouwer' ), $field['label'] ) .'</a>';
			echo '</td>';
			echo '</tr>';
			echo '</tbody>';
		}
	...
	static public function render_settings_form($type = null, $settings = null)
	{
		...
		$form       = EPBOUWERModel::get_settings_form( $type );

		if(isset($settings) && !empty($settings)) {
			$defaults = EPBOUWERModel::get_settings_form_defaults( $type );
			$settings = (object)array_merge((array)$defaults, (array)$settings);
		}
		else {
			$settings = EPBOUWERModel::get_settings_form_defaults( $type );
		}
	...
	{
		$settings = EPBOUWERModel::get_global_settings();
		$form = EPBOUWERModel::$settings_forms['global'];

		self::render_settings(array(
			'class'   => 'ep-bouwer-global-settings',
			'title'   => $form['title'],
			'tabs'    => $form['tabs']
		), $settings);
	}
	...
	static public function register_templates_post_type()
	{
		...
		register_post_type('ep-bouwer-template', array(
			'public'            => EPBOUWERModel::is_user_templates_admin_enabled() ? true : false,
			'labels'            => array(
				'name'               => _x( 'Templates', 'Custom post type label.', 'ep-bouwer' ),
				'singular_name'      => _x( 'Template', 'Custom post type label.', 'ep-bouwer' ),
				'menu_name'          => _x( 'Templates', 'Custom post type label.', 'ep-bouwer' ),
				'name_admin_bar'     => _x( 'Template', 'Custom post type label.', 'ep-bouwer' ),
				'add_new'            => _x( 'Add New', 'Custom post type label.', 'ep-bouwer' ),
				'add_new_item'       => _x( 'Add New Template', 'Custom post type label.', 'ep-bouwer' ),
				'new_item'           => _x( 'New Template', 'Custom post type label.', 'ep-bouwer' ),
				'edit_item'          => _x( 'Edit Template', 'Custom post type label.', 'ep-bouwer' ),
				'view_item'          => _x( 'View Template', 'Custom post type label.', 'ep-bouwer' ),
				'all_items'          => _x( 'All Templates', 'Custom post type label.', 'ep-bouwer' ),
				'search_items'       => _x( 'Search Templates', 'Custom post type label.', 'ep-bouwer' ),
				'parent_item_colon'  => _x( 'Parent Templates:', 'Custom post type label.', 'ep-bouwer' ),
				'not_found'          => _x( 'No templates found.', 'Custom post type label.', 'ep-bouwer' ),
				'not_found_in_trash' => _x( 'No templates found in Trash.', 'Custom post type label.', 'ep-bouwer' )
			),
			'menu_icon'			=> 'dashicons-welcome-widgets-menus',
			'supports'          => $supports,
			'taxonomies'		=> array(
				'ep-bouwer-template-category'
			),
		...
		register_taxonomy( 'ep-bouwer-template-category', array( 'ep-bouwer-template' ), array(
			'labels'            => array(
				'name'              => _x( 'Categories', 'Custom taxonomy label.', 'ep-bouwer' ),
				'singular_name'     => _x( 'Category', 'Custom taxonomy label.', 'ep-bouwer' ),
				'search_items'      => _x( 'Search Categories', 'Custom taxonomy label.', 'ep-bouwer' ),
				'all_items'         => _x( 'All Categories', 'Custom taxonomy label.', 'ep-bouwer' ),
				'parent_item'       => _x( 'Parent Category', 'Custom taxonomy label.', 'ep-bouwer' ),
				'parent_item_colon' => _x( 'Parent Category:', 'Custom taxonomy label.', 'ep-bouwer' ),
				'edit_item'         => _x( 'Edit Category', 'Custom taxonomy label.', 'ep-bouwer' ),
				'update_item'       => _x( 'Update Category', 'Custom taxonomy label.', 'ep-bouwer' ),
				'add_new_item'      => _x( 'Add New Category', 'Custom taxonomy label.', 'ep-bouwer' ),
				'new_item_name'     => _x( 'New Category Name', 'Custom taxonomy label.', 'ep-bouwer' ),
				'menu_name'         => _x( 'Categories', 'Custom taxonomy label.', 'ep-bouwer' ),
			),
			'hierarchical'      => true,
			'public'            => true,
	...
	{
		if(file_exists(EP_BOUWER_DIR . 'includes/template-selector.php')) {

			$enabled_templates  = EPBOUWERModel::get_enabled_templates();
			$user_templates     = EPBOUWERModel::get_user_templates();
			$templates          = EPBOUWERModel::get_template_selector_data();

			include EP_BOUWER_DIR . 'includes/template-selector.php';
	...
	{
		$defaults = EPBOUWERModel::get_settings_form_defaults( 'user_template' );
		$form     = EPBOUWERModel::get_settings_form( 'user_template' );

		EPBOUWER::render_settings(array(
			'class'   => 'ep-bouwer-user-template-settings',
			'title'   => $form['title'],
			'tabs'    => $form['tabs']
		), $defaults);
	}
	...
	static public function render_template($template)
	{
		...
		if($post && $post->post_type == 'ep-bouwer-template') {
	...
	{
		$icon_sets = EPBOUWERIcons::get_sets();

		include EP_BOUWER_DIR . 'includes/icon-selector.php';
	...
	static public function render_row($row)
	{
		...
		include EP_BOUWER_DIR . 'includes/row.php';
	}
	...
	static public function render_new_row($cols = '1-col', $position = false)
	{
		...
		$row        = EPBOUWERModel::add_row($cols, $position);
	...
	{
		$custom_class = apply_filters( 'ep_bouwer_row_custom_class', $row->settings->class, $row );
	...
	static public function render_row_bg($row)
	{
		if($row->settings->bg_type == 'video') {
			...
			$vid_data = EPBOUWERModel::get_row_bg_data($row);

			if($vid_data) {
				include EP_BOUWER_DIR . 'includes/row-video.php';
			}
		}
	...
	static public function render_row_settings($node_id = null)
	{
		...
		$node       = EPBOUWERModel::get_node($node_id);
		$settings   = $node->settings;
		$form       = EPBOUWERModel::$settings_forms['row'];

		self::render_settings(array(
			'class'     => 'ep-bouwer-row-settings',
	...
	static public function render_column_group($group)
	{
		...
		include EP_BOUWER_DIR . 'includes/column-group.php';
	}
	...
	static public function render_new_column_group($node_id = null, $cols = '1-col', $position = false)
	{
		...
		$cols       = isset($post_data['cols']) ? $post_data['cols'] : $cols;
		$position   = isset($post_data['position']) ? (int)$post_data['position'] : $position;
		$group      = EPBOUWERModel::add_col_group($node_id, $cols, $position);
	...
	static public function render_column_settings($node_id = null)
	{
		...
		$node       = EPBOUWERModel::get_node($node_id);
		$settings   = $node->settings;
		$form       = EPBOUWERModel::$settings_forms['col'];

		self::render_settings(array(
			'class'     => 'ep-bouwer-col-settings',
			'attrs'     => 'data-node="'. $node->node .'"',
			'title'     => $form['title'],
			'tabs'      => $form['tabs']
		), $settings);
	...
	{
		$custom_class = apply_filters( 'ep_bouwer_column_custom_class', $col->settings->class, $col );
	...
	{
		$modules = EPBOUWERModel::get_modules($parent_id);

		foreach($modules as $module) {
			$settings = $module->settings;
			include EP_BOUWER_DIR . 'includes/module.php';
		}
	}
	...
	static public function render_new_module_settings($parent_id = null, $type = null, $position = false)
	{
		...
		$type           = isset($post_data['type']) ? $post_data['type'] : $type;
		$position       = isset($post_data['position']) ? (int)$post_data['position'] : $position;
		$module         = EPBOUWERModel::add_default_module($parent_id, $type, $position);

		// Force the global parent id.
		EPBOUWERModel::update_post_data('parent_id', $module->parent);
	...
	static public function render_module_settings($node_id = null, $type = null, $parent_id = null, $return = false)
	{
		...
		if($node_id) {
			...
			$module     = EPBOUWERModel::$modules[$type];
			$settings   = EPBOUWERModel::get_module_defaults($type);
		}

		if(file_exists($module->dir .'css/settings.css')) {
			echo '<link class="ep-bouwer-settings-css" rel="stylesheet" href="'. $module->url .'css/settings.css" />';
		}
		if(file_exists($module->dir .'js/settings.js')) {
			echo '<script class="ep-bouwer-settings-js" src="'. $module->url .'js/settings.js"></script>';
		}

		echo self::render_settings(array(
			'class' => 'ep-bouwer-module-settings ep-bouwer-'. $type .'-settings',
			'attrs' => 'data-node="'. $node_id .'" data-parent="'. $parent_id .'" data-type="'. $type .'"',
			'title' => sprintf( _x( '%s Settings', '%s stands for module name.', 'ep-bouwer' ), $module->name ),
			'tabs'  => $module->form
		), $settings, $return);
	}
	...
	{
		// Settings
		$defaults = EPBOUWERModel::get_module_defaults($type);
		$settings = (object)array_merge((array)$defaults, (array)$settings);

		// Module
		$class = get_class(EPBOUWERModel::$modules[$type]);
		$module = new $class();
	...
	{
		$custom_class = apply_filters( 'ep_bouwer_module_custom_class', $module->settings->class, $module );
	...
	{
		// Delete the old file.
		EPBOUWERModel::delete_asset_cache('css');

		// Get info on the new file.
		$rows               = EPBOUWERModel::get_nodes('row');
		$cols               = EPBOUWERModel::get_nodes('column');
		$modules            = EPBOUWERModel::get_all_modules();
		$global_settings    = EPBOUWERModel::get_global_settings();
		$asset_info         = EPBOUWERModel::get_asset_info();
		$post_id            = EPBOUWERModel::get_post_id();
		...
		// Global css
		$css = file_get_contents(EP_BOUWER_DIR . '/css/ep-bouwer-layout.css');
		
		// Global RTL css
		if(is_rtl()) {
			$css .= file_get_contents(EP_BOUWER_DIR . '/css/ep-bouwer-layout-rtl.css');
		}
		...
		if($global_settings->responsive_enabled) {
			...
			$css .= file_get_contents(EP_BOUWER_DIR . '/css/ep-bouwer-layout-medium.css');
			$css .= ' }';
			$css .= '@media (max-width: '. $global_settings->responsive_breakpoint .'px) { ';
			$css .= file_get_contents(EP_BOUWER_DIR . '/css/ep-bouwer-layout-responsive.css');
			
			if ( ! isset( $global_settings->auto_spacing ) || $global_settings->auto_spacing ) {
				$css .= file_get_contents(EP_BOUWER_DIR . '/css/ep-bouwer-layout-auto-spacing.css');
			}
		...
		foreach($rows as $row) {
			...
			// Instance row css
			ob_start();
			include EP_BOUWER_DIR . 'includes/row-css.php';
			$css .= ob_get_clean();
		...
		foreach($cols as $col) {
			...
			// Instance column css
			ob_start();
			include EP_BOUWER_DIR . 'includes/column-css.php';
			$css .= ob_get_clean();
		...
		if($post && !$global_settings->show_default_heading && ($post->post_type == 'page' || $post->post_type == 'ep-bouwer-template')) {
	...
	{
		// Delete the old file.
		EPBOUWERModel::delete_asset_cache('js');
		...
		// Main JS
		$js .= file_get_contents(EP_BOUWER_DIR . 'js/ep-bouwer-layout.js');
		...
		foreach($rows as $row) {
			...
			ob_start();
			include EP_BOUWER_DIR . 'includes/row-js.php';
			$js .= ob_get_clean();
		}
		...
		$js .= "var flBuilderUrl = '" . EP_BOUWER_URL . "';";

		// Call the EPBOUWER._renderLayoutComplete method if we're currently editing.
		if(stristr($asset_info['js'], '-draft.js') || stristr($asset_info['js'], '-preview.js')) {
			$js .= "; if(typeof EPBOUWER !== 'undefined' && typeof EPBOUWER._renderLayoutComplete !== 'undefined') EPBOUWER._renderLayoutComplete();";
		}

		// Include FLJSMin
		if(!class_exists('FLJSMin')) {
			include EP_BOUWER_DIR . 'classes/class-fl-jsmin.php';
		}
