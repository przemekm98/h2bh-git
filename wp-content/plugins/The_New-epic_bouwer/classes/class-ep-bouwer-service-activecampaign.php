 ...
 * @since 1.6.0
 */
final class EPBOUWERServiceActiveCampaign extends EPBOUWERService {
	...
	public function get_api( $api_url, $api_key ) 
	{
		...
		if ( ! class_exists( 'ActiveCampaign' ) ) {
			require_once EP_BOUWER_DIR . 'includes/vendor/activecampaign/ActiveCampaign.class.php';
		}
	...
	public function connect( $fields = array() ) 
	{
		...
		if ( ! isset( $fields['api_url'] ) || empty( $fields['api_url'] ) ) {
			$response['error'] = __( 'Error: You must provide an API URL.', 'ep-bouwer' );
		}
		...
		// Try to connect and store the connection data.
			...
			if ( ! (int) $api->credentials_test() ) {
				$response['error'] = __( 'Error: Please check your API URL and API key.', 'ep-bouwer' );
			}
			else {
				$response['data'] = array( 
	...
	public function render_connect_settings() 
	{
		...
		EPBOUWER::render_settings_field( 'api_url', array(
			'row_class'     => 'ep-bouwer-service-connect-row',
			'class'         => 'ep-bouwer-service-connect-input',
			'type'          => 'text',
			'label'         => __( 'API URL', 'ep-bouwer' ),
			'help'          => __( 'Your API url can be found in your ActiveCampaign account under My Settings > API.', 'ep-bouwer' ),
			'preview'       => array(
				'type'          => 'none'
			)
		));
		...
		EPBOUWER::render_settings_field( 'api_key', array(
			...
			'help'          => __( 'Your API key can be found in your ActiveCampaign account under My Settings > API.', 'ep-bouwer' ),
			'preview'       => array(
				'type'          => 'none'
			)
		)); 
	...
	public function subscribe( $settings, $email, $name = false )
	{
		...
		if ( ! $account_data ) {
			$response['error'] = __( 'There was an error subscribing to ActiveCampaign. The account is no longer connected.', 'ep-bouwer' );
		}
		else {
			...
			if ( ! $result->success && isset( $result->error ) && ! stristr( $result->error, 'duplicate' ) ) {
				...
				if ( stristr( $result->error, 'access' ) ) {
					$response['error'] = __( 'Error: Invalid API data.', 'ep-bouwer' );
				}
				else {
