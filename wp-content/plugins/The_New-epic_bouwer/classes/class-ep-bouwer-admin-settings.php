 ...
 * @since 1.0
 */
final class EPBOUWERAdminSettings {
	...
	{
		add_action( 'admin_menu', 'EPBOUWERAdminSettings::menu' );
			
		if ( isset( $_REQUEST['page'] ) && 'ep-bouwer-settings' == $_REQUEST['page'] ) {
			add_action( 'admin_enqueue_scripts', 'EPBOUWERAdminSettings::styles_scripts' );
			self::save();
		}
	}
	...
	{
		// Styles
		wp_enqueue_style( 'ep-bouwer-admin-settings', EP_BOUWER_URL . 'css/ep-bouwer-admin-settings.css', array(), EP_BOUWER_VERSION );

		// Scripts
		wp_enqueue_script( 'ep-bouwer-admin-settings', EP_BOUWER_URL . 'js/ep-bouwer-admin-settings.js', array(), EP_BOUWER_VERSION );
	...
	static public function menu() 
	{
		if ( current_user_can( 'delete_users' ) ) {
			...
			$title = EPBOUWERModel::get_branding();
			$cap   = 'delete_users';
			$slug  = 'ep-bouwer-settings';
			$func  = 'EPBOUWERAdminSettings::render';
	...
	{
		include EP_BOUWER_DIR . 'includes/admin-settings-js-config.php';
		include EP_BOUWER_DIR . 'includes/admin-settings.php';
	}
	...
	{
		$icon = EPBOUWERModel::get_branding_icon();
		$name = EPBOUWERModel::get_branding();
		
		if ( ! empty( $icon ) ) {
			echo '<img src="' . $icon . '" />';
		}
		
		echo '<span>' . sprintf( _x( '%s Settings', '%s stands for custom branded "Page Builder" name.', 'ep-bouwer' ), EPBOUWERModel::get_branding() ) . '</span>';
	}
	...
	static public function render_update_message()
	{
		if ( ! empty( self::$errors ) ) {
			...
			echo '<div class="updated"><p>' . __( 'Settings updated!', 'ep-bouwer' ) . '</p></div>';
		}
	}
	...
	static public function render_nav_items()
	{
		$item_data = array(
			'license' => array(
				'title' => __( 'License', 'ep-bouwer' ),
				'show'	=> EP_BOUWER_LITE !== true && ( is_network_admin() || ! self::multisite_support() )
			),
			'upgrade' => array(
				'title' => __( 'Upgrade', 'ep-bouwer' ),
				'show'	=> EP_BOUWER_LITE === true
			),
			'modules' => array(
			...
			'templates' => array(
				'title' => __( 'Templates', 'ep-bouwer' ),
				'show'	=> EP_BOUWER_LITE !== true
			),
			'post-types' => array(
				'title' => __( 'Post Types', 'ep-bouwer' ),
				'show'	=> true
			),
			'icons' => array(
	...
	static public function render_form( $type )
	{
		if ( self::has_support( $type ) ) {
			include EP_BOUWER_DIR . 'includes/admin-settings-' . $type . '.php';
		}
	}
	...
	{
		if ( is_network_admin() ) {
			echo network_admin_url( '/settings.php?page=ep-bouwer-multisite-settings#' . $type );
		}
		else {
			echo admin_url( '/options-general.php?page=ep-bouwer-settings#' . $type );
		}
	}
	...
	{
		if ( is_network_admin() ) {
			return network_admin_url( '/settings.php?page=ep-bouwer-multisite-settings#' . $type );
		}
		else {
			return admin_url( '/options-general.php?page=ep-bouwer-settings#' . $type );
		}
	}
	...
	{
		return file_exists( EP_BOUWER_DIR . 'includes/admin-settings-' . $type . '.php' );
	}
	...
	{
		return is_multisite() && class_exists( 'EPBOUWERMultisiteSettings' );
	}
	...
	static private function save_enabled_modules()
	{
		if ( isset( $_POST['fl-modules-nonce'] ) && wp_verify_nonce( $_POST['fl-modules-nonce'], 'modules' ) ) {
			...
			EPBOUWERModel::update_admin_settings_option( '_ep_bouwer_enabled_modules', $modules, true );
		}
	}
	...
	static private function save_enabled_templates()
	{
		if ( isset( $_POST['fl-templates-nonce'] ) && wp_verify_nonce( $_POST['fl-templates-nonce'], 'templates' ) ) {
			...
			EPBOUWERModel::update_admin_settings_option( '_ep_bouwer_enabled_templates', $enabled_templates, true );
			EPBOUWERModel::update_admin_settings_option( '_ep_bouwer_user_templates_admin', $admin_ui_enabled, true );
			
			if ( class_exists( 'EPBOUWERTemplatesOverride' ) ) {
				EPBOUWERTemplatesOverride::save_admin_settings();	
			}
		}
	}
	...
	static private function save_enabled_post_types()
	{
		if ( isset( $_POST['fl-post-types-nonce'] ) && wp_verify_nonce( $_POST['fl-post-types-nonce'], 'post-types' ) ) {
			...
			EPBOUWERModel::update_admin_settings_option( '_ep_bouwer_post_types', $post_types, true );
		}
	}
	...
	static private function save_enabled_icons()
	{
		if ( isset( $_POST['fl-icons-nonce'] ) && wp_verify_nonce( $_POST['fl-icons-nonce'], 'icons' ) ) {
			...
			if ( ! isset( $_POST['fl-enabled-icons'] ) && empty( $_POST['fl-new-icon-set'] ) ) {
				self::add_error( __( "Error! You must have at least one icon set enabled.", 'ep-bouwer' ) );
				return;
			}
			...
			if ( ! empty( $_POST['fl-new-icon-set'] ) ) {
				...
				$dir		 = EPBOUWERModel::get_cache_dir( 'icons' );
				...
				// Unzip failed.
				if ( ! $unzipped ) {
					self::add_error( __( "Error! Could not unzip file.", 'ep-bouwer' ) );
					return;
				}
				...
				if ( ! $is_icomoon && ! $is_fontello ) {
					...
					self::add_error( __( "Error! Please upload an icon set from either Icomoon or Fontello.", 'ep-bouwer' ) );
					return;
				}
				
				// Enable the new set. 
				if ( is_array( $enabled_icons ) ) {
					$key = EPBOUWERIcons::get_key_from_path( $new_path );
					$enabled_icons[] = $key;
				}
			}
	...
	{
		EPBOUWERModel::update_admin_settings_option( '_ep_bouwer_enabled_icons', $enabled_icons, true );
	}
	...
	static private function save_editing_capability()
	{
		if ( isset( $_POST['fl-editing-nonce'] ) && wp_verify_nonce( $_POST['fl-editing-nonce'], 'editing' ) ) {
			...
			EPBOUWERModel::update_admin_settings_option( '_ep_bouwer_editing_capability', $capability, true );
		}
	}
	...
	static private function save_branding()
	{
		if ( isset( $_POST['fl-branding-nonce'] ) && wp_verify_nonce( $_POST['fl-branding-nonce'], 'branding' ) ) {
			...
			EPBOUWERModel::update_admin_settings_option( '_ep_bouwer_branding', $branding, false );
			EPBOUWERModel::update_admin_settings_option( '_ep_bouwer_branding_icon', $branding_icon, false );
		}
	}
	...
	static private function save_help_button()
	{
		if ( isset( $_POST['fl-help-button-nonce'] ) && wp_verify_nonce( $_POST['fl-help-button-nonce'], 'help-button' ) ) {
			...
			$settings					= EPBOUWERModel::get_help_button_defaults();
			...
			if ( ! empty( $video_embed ) && ! stristr( $video_embed, 'iframe' ) ) {
				self::add_error( __( "Error! Please enter an iframe for the video embed code.", 'ep-bouwer' ) );
			}
			...
			if ( $settings['enabled'] && ! $settings['tour'] && ! $settings['video'] && ! $settings['knowledge_base'] && ! $settings['forums'] ) {
				self::add_error( __( "Error! You must have at least one feature of the help button enabled.", 'ep-bouwer' ) );
				return;
			}
			
			EPBOUWERModel::update_admin_settings_option( '_ep_bouwer_help_button', $settings, false );
		}
	}
	...
	static private function uninstall()
	{
		if ( ! current_user_can( 'delete_plugins' ) ) {
			...
			if ( is_multisite() && class_exists( 'EPBOUWERMultisite' ) ) {
				EPBOUWERMultisite::uninstall();
			}
			else {
				EPBOUWERAdmin::uninstall();
			}
		}
	}
}