 ...
 * @since 1.5.4
 */
final class EPBOUWERServiceConstantContact extends EPBOUWERService {
	...
	public function connect( $fields = array() ) 
	{
		...
		if ( ! isset( $fields['api_key'] ) || empty( $fields['api_key'] ) ) {
			$response['error'] = __( 'Error: You must provide an API key.', 'ep-bouwer' );
		}
		// Make sure we have an access token.
		else if ( ! isset( $fields['access_token'] ) || empty( $fields['access_token'] ) ) {
			$response['error'] = __( 'Error: You must provide an access token.', 'ep-bouwer' );
		}
		// Try to connect and store the connection data.
			...
			if ( ! is_array( $request ) || ( isset( $request[0] ) && isset( $request[0]->error_message ) ) ) {
				$response['error'] = sprintf( __( 'Error: Could not connect to Constant Contact. %s', 'ep-bouwer' ), $request[0]->error_message );
			}
			else {
				$response['data'] = array( 
	...
	public function render_connect_settings() 
	{
		...
		EPBOUWER::render_settings_field( 'api_key', array(
			'row_class'     => 'ep-bouwer-service-connect-row',
			'class'         => 'ep-bouwer-service-connect-input',
			'type'          => 'text',
			'label'         => __( 'API Key', 'ep-bouwer' ),
			'help'          => __( 'Your Constant Contact API key.', 'ep-bouwer' ),
			'preview'       => array(
				'type'          => 'none'
			)
		)); 
		
		EPBOUWER::render_settings_field( 'access_token', array(
			'row_class'     => 'ep-bouwer-service-connect-row',
			'class'         => 'ep-bouwer-service-connect-input',
			'type'          => 'text',
			'label'         => __( 'Access Token', 'ep-bouwer' ),
			'help'          => __( 'Your Constant Contact access token.', 'ep-bouwer' ),
			'description'   => sprintf( __( 'You must register a <a%s>Developer Account</a> with Constant Contact to obtain an API key and access token. Please see <a%s>Getting an API key</a> for complete instructions.', 'ep-bouwer' ), ' href="https://constantcontact.mashery.com/member/register" target="_blank"', ' href="https://developer.constantcontact.com/home/api-keys.html" target="_blank"' ),
			'preview'       => array(
				'type'          => 'none'
			)
		)); 
	...
	public function subscribe( $settings, $email, $name = false )
	{
		...
		if ( ! $account_data ) {
			$response['error'] = __( 'There was an error subscribing to Constant Contact. The account is no longer connected.', 'ep-bouwer' );
		}
		else {
			...
			if ( ! empty( $contact->results ) ) {
				...
				if ( isset( $res->error_key ) ) {
					$response['error'] = sprintf( __( 'There was an error subscribing to Constant Contact. %s', 'ep-bouwer' ), $res->error_key );
				}
			}
			// Add a new contact.
			else {
				...
				if ( isset( $create->error_key ) ) {
					$response['error'] = sprintf( __( 'There was an error subscribing to Constant Contact. %s', 'ep-bouwer' ), $create->error_key );
				}
			}
		}
