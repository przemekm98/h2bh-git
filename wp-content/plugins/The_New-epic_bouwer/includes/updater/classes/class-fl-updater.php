...
/**
 * Manages remote updates for all Epic Bouwer products.
 *
 * @since 1.0
 */
final class FLUpdater {
	...
	/**
	 * The API URL for the Epic Bouwer store. 
	 *
	 * @since 1.0
	 * @access private
	 * @var string $_store_api_url
	 */
	static private $_store_api_url = 'http://www.epicwebsite.nl/epicbouwer/';

	/**
	 * The API URL for the Epic Bouwer update server. 
	 *
	 * @since 1.0
	 * @access private
	 * @var string $_updates_api_url
	 */
	static private $_updates_api_url = 'http://updates.epicbouwer.nl/';
	...
	public function update_message( $plugin_data, $response )
	{
		if ( empty( $response->package ) ) {
			...
			echo __( '<strong>UPDATE UNAVAILABLE!</strong>', 'ep-bouwer' );
			echo '&nbsp;&nbsp;&nbsp;';
			echo __('Please subscribe to enable automatic updates for this plugin.', 'ep-bouwer');
			echo ' <a href="' . $plugin_data['PluginURI'] . '" target="_blank" style="color: #fff; text-decoration: underline;">';
			echo __('Subscribe Now', 'ep-bouwer');
			echo ' &raquo;</a>';
			echo '</p>';
		}
	}
