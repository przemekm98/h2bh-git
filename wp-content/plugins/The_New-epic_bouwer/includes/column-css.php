...
.fl-node-<?php echo $col->node; ?> .fl-col-content {
	...
	background-color: rgba(<?php echo implode(',', EPBOUWERColor::hex_to_rgb($col->settings->bg_color)) ?>, <?php echo $col->settings->bg_opacity/100; ?>);
}
<?php endif; ?>
...
.fl-node-<?php echo $col->node; ?> .fl-col-content:after {
	...
	background-color: rgba(<?php echo implode( ',', EPBOUWERColor::hex_to_rgb( $col->settings->bg_overlay_color ) ) ?>, <?php echo $col->settings->bg_overlay_opacity/100; ?>);
}
<?php endif; ?>

<?php if(!empty($col->settings->border_type)) : // Border ?>
.ep-bouwer-content .fl-node-<?php echo $col->node; ?> .fl-col-content {
	border-style: <?php echo $col->settings->border_type; ?>;
	border-color: #<?php echo $col->settings->border_color; ?>;
	border-color: rgba(<?php echo implode(',', EPBOUWERColor::hex_to_rgb($col->settings->border_color)) ?>, <?php echo $col->settings->border_opacity/100; ?>);
...
<?php if($global_settings->responsive_enabled) : // Responsive Sizes ?>
	...
	@media(max-width: <?php echo $global_settings->medium_breakpoint; ?>px) {
		.ep-bouwer-content .fl-node-<?php echo $col->node; ?> {
