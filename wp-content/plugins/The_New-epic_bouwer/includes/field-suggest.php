...
?>
<input type="text" name="<?php echo $name; ?>" data-value='<?php echo EPBOUWERAutoSuggest::get_value($action, $value, $data); ?>' data-action="<?php echo $action; ?>" data-action-data="<?php echo $data; ?>" class="text text-full fl-suggest-field<?php if(isset($field['class'])) echo ' '. $field['class']; ?>" placeholder="<?php if ( isset( $field['placeholder'] ) ) echo esc_attr( $field['placeholder'] ); else esc_attr_e( 'Start typing...', 'ep-bouwer' ); ?>" />