...
<?php else : ?>
	<th>
		...
		if ( 'button' == $field['type'] ) {
			...
			if ( isset( $i ) ) {
				echo ' <span class="ep-bouwer-field-index">' . ( $i + 1 ) . '</span>';
			}    
		}
...
<?php endif; ?>
	...
	$field_file = EP_BOUWER_DIR . 'includes/field-' . $field['type'] . '.php';
	
	if(file_exists($field_file)) {
		include $field_file;
	}
	else {
		do_action('ep_bouwer_control_' . $field['type'], $name, $value, $field);
	}
