...
if ( ! empty( $settings->bg_color ) ) {
	$border_color = EPBOUWERColor::adjust_brightness( $settings->bg_color, 12, 'darken' );
}
if ( ! empty( $settings->bg_hover_color ) ) {
	$border_hover_color = EPBOUWERColor::adjust_brightness( $settings->bg_hover_color, 12, 'darken' );
}
...
// Background Gradient
if ( ! empty( $settings->bg_color ) ) {
	$bg_grad_start = EPBOUWERColor::adjust_brightness( $settings->bg_color, 30, 'lighten' );
}
if ( ! empty( $settings->bg_hover_color ) ) {
	$bg_hover_grad_start = EPBOUWERColor::adjust_brightness( $settings->bg_hover_color, 30, 'lighten' );
}

?>
.ep-bouwer-content .fl-node-<?php echo $id; ?> a.fl-button,
.ep-bouwer-content .fl-node-<?php echo $id; ?> a.fl-button:visited {
	...
	border: <?php echo $border_size; ?>px solid #<?php echo $border_color; ?>;
		...
		background-color: rgba(<?php echo implode( ',', EPBOUWERColor::hex_to_rgb( $settings->bg_color ) ) ?>, <?php echo $settings->bg_opacity/100; ?>);
		<?php endif; ?>
...
.ep-bouwer-content .fl-node-<?php echo $id; ?> a.fl-button:hover,
.ep-bouwer-content .fl-node-<?php echo $id; ?> a.fl-button:focus {

	background: #<?php echo $settings->bg_hover_color; ?>;
	border: <?php echo $border_size; ?>px solid #<?php echo $border_hover_color; ?>;
	
	<?php if ( 'transparent' == $settings->style ) : // Transparent ?>
	background-color: rgba(<?php echo implode( ',', EPBOUWERColor::hex_to_rgb( $settings->bg_hover_color ) ) ?>, <?php echo $settings->bg_opacity/100; ?>);
	<?php endif; ?>
